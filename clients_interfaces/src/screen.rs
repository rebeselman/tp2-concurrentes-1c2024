...
use std::{collections::HashMap, env, error::Error, fs::File, io::{BufRead, BufReader}, net::UdpSocket, sync::{atomic::AtomicBool, Arc, Condvar, Mutex}, thread, time::{Duration, Instant}};
use std::sync::MutexGuard;
use actix::{Actor, Context, Handler};
use orders::order::Order;

//use clients_interfaces::screen_message::ScreenMessage;
use clients_interfaces::{order_state::OrderState, screen_message::ScreenMessage, screen_state::ScreenState};
...
impl Screen {
    ...
    fn broadcast_and_wait(&mut self, message: &[u8], expected: OrderState, order: &Order) -> Result<bool, Box<dyn Error>> {
        {
    ...
    fn handle(&mut self, msg: ScreenMessage, _ctx: &mut Context<Self>) {
        match msg {
            ScreenMessage::Ping { screen_id } => {
                println!("[SCREEN{}] received PING MESSAGE FROM {}", self.id,  screen_id);
        ...
        match response {
            "ready" => {
                ...
                if from.to_string() == PAYMENT_GATEWAY_IP {
                    ...
                    println!("[SCREEN {}] received READY from payment gateway for order {}", screen.id, order_id);
                } else {
                    responses[ORDER_MANAGEMENT] = Some(OrderState::Ready);
                    println!("[SCREEN {}] received READY from order management for order {}", screen.id, order_id);
            ...
            "abort" => {
                ...
                if from.to_string() == PAYMENT_GATEWAY_IP {
                    ...
                    println!("[SCREEN {}] received ABORT from payment gateway for order {}", screen.id, order_id);
                } else {
                    responses[ORDER_MANAGEMENT] = Some(OrderState::Abort);
                    println!("[SCREEN {}] received ABORT from order management for order {}", screen.id, order_id);
                }
                screen.responses.1.notify_all();
            }
            "finished" => {
                ...
                if from.to_string() == PAYMENT_GATEWAY_IP {
                    ...
                    println!("[SCREEN {}] received FINISHED from payment gateway for order {}", screen.id, order_id);
                } else {
                    responses[ORDER_MANAGEMENT] = Some(OrderState::Finished);
                    println!("[SCREEN {}] received FINISHED from order management for order {}", screen.id, order_id);
                }
                screen.responses.1.notify_all();
            }
            "keepalive" => {
                ...
                if from.to_string() == PAYMENT_GATEWAY_IP {
                    ...
                    println!("[SCREEN {}] received KEEPALIVE from payment gateway for order {}", screen.id, order_id);
                } else {
                    responses[ORDER_MANAGEMENT] = Some(OrderState::Wait(Instant::now()));
                    println!("[SCREEN {}] received KEEPALIVE from order management for order {}", screen.id, order_id);
                }
                screen.responses.1.notify_all();
            ...
            "screen" => {
                ...
                let message: ScreenMessage = serde_json::from_str(parts.next().ok_or("No message")?)?;
