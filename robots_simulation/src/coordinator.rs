...
use robots_simulation::coordinator_messages::CoordinatorMessage::{
    self, AccessAllowed, AccessDenied, OrderReceived,
};
...
impl Coordinator {
    ...
    async fn assign_order_to_robot(&mut self, order: Order) {
        ...
        if let Some((&robot_id, &_available)) = robot_states
            .iter()
            .find(|&(_, &available)| available == false)
        {
            ...
            self.socket
                .send_to(&msg, format!("0.0.0.0:809{}", robot_id))
                .await
                .unwrap();
    ...
    async fn process_queue(&self) {
        ...
        while let Some((flavors, robot_id, robot_addr)) = flavor_requests_queue.pop_front() {
            self.check_if_flavor_available(robot_id, &flavors, robot_addr)
                .await;
        }
    }
    ...
    async fn check_if_flavor_available(
        ...
        for flavor in flavors {
            ...
            let response = if !*access {
                *access = true;
                println!(
                    "[COORDINATOR] Robot {} is requesting access to container {:?}",
                    robot_id, flavor
                );
                ...
                AccessDenied {
                    reason: "Container already in use".into(),
                }
            };
        ...
        let response = AccessDenied {
            reason: "All requested containers are in use".into(),
        };
        self.flavor_requests
            .lock()
            .await
            .push_back((flavors.clone(), robot_id, addr));
        send_response(&self.socket, &response, addr).await;
    }
                ...
                if let Some(order_state) = self.orders.remove(&order.id()) {
                    ...
                    actix_rt::spawn(async move {
                        ...
                        if let Some(robot_id) = order_state.robot_id {
                            ...
                            let msg = serde_json::to_vec(&CoordinatorMessage::OrderAborted {
                                robot_id,
                                order,
                            })
                            .unwrap();
                        ...
                        let addr: SocketAddr = SocketAddr::new(
                            order_state.screen_addr.ip(),
                            order_state.screen_addr.port(),
                        );
    ...
    fn handle(&mut self, msg: RobotResponse, _ctx: &mut Self::Context) -> Self::Result {
        match msg {
            RobotResponse::AccessRequest {
                ...
                let this = self.clone();
                actix_rt::spawn(async move {
                    this.check_if_flavor_available(robot_id, &flavors, addr)
                        .await;
                });
            }
            RobotResponse::ReleaseRequest {
                ...
                let this = self.clone();
                println!(
                    "[COORDINATOR] Releasing access for container {:?} from {:?}",
                    flavor, robot_id
                );
                actix_rt::spawn(async move {
    ...
    loop {
        ...
        match message_type {
            "prepare" => {
                ...
                println!(
                    "[COORDINATOR] Received prepare message for order: {}",
                    order.id()
                );
            ...
            "commit" => {
                let order: Order = serde_json::from_str(&parts.next().unwrap()).unwrap();
                println!(
                    "[COORDINATOR] Received commit message for order: {}",
                    order.id()
                );
            ...
            "abort" => {
                let order: Order = serde_json::from_str(&parts.next().unwrap()).unwrap();
                println!(
                    "[COORDINATOR] Received abort message for order: {}",
                    order.id()
                );
