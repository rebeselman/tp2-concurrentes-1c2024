...
use rand::Rng;
use robots_simulation::lib::items::{ContainerType, IceCreamFlavor, Item, Order, RequestToCoordinator, Response};
...
fn main() -> io::Result<()> {
    ...
    loop {
        ...
        match socket.recv_from(&mut buf) {
            Ok((amt, recv_addr)) => {
                println!("Received message from {}", recv_addr);
                let request: RequestToCoordinator = serde_json::from_slice(&buf[..amt]).unwrap();
                match request {
                    RequestToCoordinator::AssignOrder { robot_id: id, mut order } => {
                        if id == robot_id {
                            make_order(robot_id, &socket, server_addr, &mut order)?;
...
fn make_order(robot_id: usize, socket: &UdpSocket, server_addr: &str, order: &mut Order) -> io::Result<()> {
    ...
    while flavors_needed.values().any(|&completed| !completed) {
        ...
        make_request(socket, server_addr, &RequestToCoordinator::SolicitarAcceso { robot_id, flavors: flavors_to_request })?;
...
fn read_coordinator_answer(robot_id: usize, socket: &UdpSocket, server_addr: &str, buf: &mut [u8; 1024], amt: &usize, order: &mut Order, flavors_needed: &mut HashMap<IceCreamFlavor, bool>) -> io::Result<()> {
...
fn send_release_request(flavor: &IceCreamFlavor, robot_id: usize, socket: &UdpSocket, server_addr: &str) -> io::Result<()> {
    ...
    let release_request = RequestToCoordinator::LiberarAcceso { robot_id, flavor: flavor.clone() };
    make_request(socket, server_addr, &release_request)?;
    let mut buf = [0; 1024];
...
fn mark_flavor_as_completed(order: &mut Order, flavor: &IceCreamFlavor) {
...
fn make_request(socket: &UdpSocket, server_addr: &str, request: &RequestToCoordinator) -> io::Result<()> {
